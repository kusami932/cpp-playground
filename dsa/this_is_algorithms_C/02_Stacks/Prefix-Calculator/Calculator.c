...
void GetPostFix(char *Infix, char *Postfix)
{
    ...
    while(Position < Length)
    {
        ...
        {
            while (!LLS_IsEmpty(Stack) &&
                    !IsPrior(LLS_Pop(Stack)->Data[0], Token[0]))
                {
                    Node *Popped = LLS_Pop(Stack);
        ...
        Read += GetNextToken(&Postfix[Read], Token, &Type);
            ...
            else
            {
                ...
                switch (Type)
                {
                    ...
                    case MULT: TempResult = Operator1 * Operator2; break;
                    case DIV: TempResult = Operator1 / Operator2; break;
                }
